...
class TestParseSushiParams(unittest.TestCase):
    """SUSHI params adapter: readable keys + template merge + fasta extraction."""

    def _write(self, root: Path, **extra) -> Path:
        p = root / "sushi_params.yml"
        p.write_text(yaml.safe_dump({"paramsTemplate": "default-DIA", **extra}))
        return p
    ...
    def test_matches_apprunner_for_equivalent_keys(self):
        """SUSHI readable keys → same DIANNRunnerParams as AppRunner's B-Fabric keys."""
        with tempfile.TemporaryDirectory() as t:
            # SUSHI side: readable keys mirroring FLAT's non-default values.
            p = self._write(
                Path(t),
            ...
            su_wf, _, _ = parse_sushi_params(p)
            ar_wf = parse_flat_params(dict(FLAT))
            self.assertEqual(su_wf["diann"]["min_pr_charge"], ar_wf["diann"]["min_pr_charge"])
            self.assertEqual(su_wf["diann"]["relaxed_prot_inf"], ar_wf["diann"]["relaxed_prot_inf"])
            self.assertEqual(su_wf["diann"]["var_mods"], ar_wf["diann"]["var_mods"])
...
class TestSushiEndToEnd(unittest.TestCase):
    ...
    def test_derives_inputs_and_materializes_workdir(self):
        with tempfile.TemporaryDirectory() as t:
            ...
            # Readable override flowed through alias → parse_flat_params → TOML.
