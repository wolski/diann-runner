...
The inputs the workflow needs are committed in this directory (``params.yml`` at
the work-dir root and ``input/raw/dataset.parquet``). This script only fetches
the large inputs that are *not* committed:
...
(The root ``dataset.csv`` is generated by snakemake's ``dataset_csv`` rule from
the committed ``dataset.parquet`` — so the test exercises that rule.
``order.fasta`` / ``inputs.yml`` from the production work dir are not committed:
the workflow doesn't use them here — ``use_custom=false`` and ``inputs.yml`` is
an app-runner artifact.)
"""
