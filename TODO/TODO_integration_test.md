...
Only the two inputs the workflow actually needs are committed (the same files bfabric-app-runner provides); the large inputs are downloaded by the setup script:

- [x] `params.yml` — flat-key Bfabric parameters (committed, at the work-dir root).
- [x] `dataset.csv` — sample-to-condition mapping (committed, at the work-dir root). app-runner provides this directly; the Snakefile's `dataset_csv` (parquet→csv) rule stays dormant, so **no `dataset.parquet` is needed** (verified: the DAG drops `dataset_csv` and builds 10 jobs).
...
> `order.fasta` and `inputs.yml` appear in `tree.txt` but are **not** committed — the workflow doesn't use them (`03_fasta_use_custom=false`; `inputs.yml` is an app-runner artifact).
...
├── dataset.csv                  # committed (app-runner provides it directly)
├── input/
...
# dataset.parquet (dataset_csv rule dormant); out-DIANN_*, logs/, Result_*.zip are generated.
...
`setup_integration_test.py`, `run.sh`, `tree.txt`, `params.yml`, `dataset.csv`.
A local `.gitignore` tracks only those and ignores all downloaded/generated
content.
