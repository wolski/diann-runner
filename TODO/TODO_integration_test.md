...
Committed at their real work-tree positions in `tests/integration/WU346549/`, or downloaded by the setup script:

- [x] `params.yml` — flat-key Bfabric parameters (committed, at the work-dir root).
- [x] `input/raw/dataset.parquet` — sample-to-condition mapping; the upstream input rule `dataset_csv` reads (committed). The root `dataset.csv` is **generated** from it by snakemake at run time (not committed).
...
`setup_integration_test.py`, `run.sh`, `tree.txt`, `params.yml`, `inputs.yml`,
`input/order.fasta`, `input/raw/dataset.parquet`. A local `.gitignore` ignores
